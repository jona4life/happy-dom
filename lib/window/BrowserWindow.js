...
export const prepareWindow = (browserFrame, options) => {
    ...
    globalThis[PropertySymbol.customElementReactionStack] = new CustomElementReactionStack(globalThis);
    ...
    globalThis['queueMicrotask'] = function (callback) {
        ...
        let isAborted = false;
        const taskId = globalThis.#browserFrame[PropertySymbol.asyncTaskManager].startTask(() => (isAborted = true));
    ...
    globalThis['fetch'] = async function (url, init) {
        if (globalThis.closed) {
            return Promise.reject(new globalThis.DOMException("Failed to execute 'fetch' on 'Window': The window is closed.", DOMExceptionNameEnum.invalidStateError));
        }
    ...
    globalThis['postMessage'] = function (message, targetOrigin = '*', _transfer) {
        ...
        if (targetOrigin && targetOrigin !== '*' && globalThis.location.origin !== targetOrigin) {
            throw new globalThis.DOMException(`Failed to execute 'postMessage' on 'Window': The target origin provided ('${targetOrigin}') does not match the recipient window\'s origin ('${globalThis.location.origin}').`, DOMExceptionNameEnum.securityError);
        }
        try {
            JSON.stringify(message);
        }
        catch (error) {
            throw new globalThis.DOMException(`Failed to execute 'postMessage' on 'Window': The provided message cannot be serialized.`, DOMExceptionNameEnum.invalidStateError);
        }
        globalThis.setTimeout(() => globalThis.dispatchEvent(new MessageEvent('message', {
            data: message,
    ...
    globalThis[PropertySymbol.location] = new Location(globalThis.#browserFrame, options?.url ?? 'about:blank');
